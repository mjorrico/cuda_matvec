...
void transpose_matrix(float* mat, int N) {
    float* clone;
    cudaMalloc(&clone, N * N * sizeof(float));
    cudaMemcpy(clone, mat, N * N * sizeof(float), cudaMemcpyDeviceToDevice);
    ...
    cublasSgeam(handle, CUBLAS_OP_T, CUBLAS_OP_N, N, N, &alpha, clone, N, &beta, clone, N, mat, N);
...
__global__ void matvecmul_mykernel_transpose(int M, int N, float* mat, float* vec_in, float* vec_out) {
    ...
    const int mat_col = blockIdx.x * WARPS_PER_BLOCK + warp_id; // one wrap handles one column
    const int thr_len = (M + WARPSIZE - 1) / WARPSIZE; //  the # of scalar multiplication performed by a single thread

    // populating shared memory
    int shmem_idx;
    for (int i = 0; i < M; i += WARPSIZE) {
        shmem_idx = i + thr_id;
        shmem_vec_in[shmem_idx] = (shmem_idx < M) ? vec_in[shmem_idx] : 0;
    }
    __syncthreads();

    int product_idx;
    mat += mat_col; // moves pointer to assigned partition
    for (int i = 0; i < thr_len; i++) {
        product_idx = i * WARPSIZE + thr_id;
        if (product_idx < M && mat_col < N) tmp += mat[product_idx * N] * shmem_vec_in[product_idx];
    }
    __syncthreads();
    ...
    if (thr_id == 0 && mat_col < N) {
        vec_out[mat_col] = tmp;
    }
}

void matvecmul_cublas_transpose(int M, int N, float* mat, float* vec_in, float* vec_out) {
    cublasHandle_t handle;
    cublasCreate(&handle);
    float alpha = 1.0, beta = 0.0;
    cublasSgemv(handle, CUBLAS_OP_N,N, M, &alpha, mat, M, vec_in, 1, &beta, vec_out, 1);
    cublasDestroy(handle);
}
...
void benchmark_matvecmul(int M, int N, long repeat) {
    ...
    if (M != N) {
        throw std::runtime_error("M must be equal to N.");
    }
    ...
    dim3 gridsize((N + WARPS_PER_BLOCK - 1) / WARPS_PER_BLOCK);

    for (unsigned int t = 0; t < n_tests; t++) {
        const auto t1 = std::chrono::steady_clock::now();
        for (unsigned rep = 0; rep < n_repeat; rep++) {
            matvecmul_cublas_transpose(M, N, mat_device, vec_in_device, vec_out_device);
            // matvecmul_mykernel_transpose<<<gridsize, blocksize, shared_memory_size>>>(M, N, mat_device, vec_in_device, vec_out_device);
    ...
    transpose_matrix(mat_device, M); // we transpose the matrix to prepare for validation because validate() only accepts matrix that has been transposed
    CUDA_CHECK(cudaMemcpy(mat_host, mat_device, M * M * sizeof(float), cudaMemcpyDeviceToHost));
